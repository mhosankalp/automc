...
def add_user(username, password, email, paymentflag):
    with session_scope() as s:
        u = tabledef.User(username=username, password=password.decode('utf8'), email=email, paymentflag=paymentflag)
        s.add(u)
        s.commit()
...
def credentials_valid(username, password):
    with session_scope() as s:
        user = s.query(tabledef.User).filter(tabledef.User.username.in_([username])).first()
        paymentflag = user.paymentflag
        if user and paymentflag=='Y':
